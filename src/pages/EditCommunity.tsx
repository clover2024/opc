...
export default function EditCommunity() {
  ...
  useEffect(() => {
    ...
    searchTimeout.current = setTimeout(() => searchAmap(searchQuery), 300);
  ...
  if (!isAdmin) {
    return (
      <div className="text-center py-16">
        <p className="text-red-600">只有管理员可以编辑社区</p>
      </div>
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8">
        <h1 className="text-2xl font-bold text-slate-900 mb-6">编辑社区</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="relative">
            ...
            {loadingSuggestions && <div className="absolute right-3 top-[38px]"><Loader2 className="w-5 h-5 animate-spin text-slate-400" /></div>}
          </div>

          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">社区名称</label>
            <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}
              className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none" required />
          </div>
          ...
          <div>
            ...
            <input type="text" value={formData.contact_wechat} onChange={(e) => setFormData({ ...formData, contact_wechat: e.target.value })}
              className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none" />
          </div>
          ...
          <div>
            ...
            <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })}
              rows={4} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none resize-none" />
          </div>

          <div className="flex gap-4 pt-4">
            <button type="button" onClick={() => navigate('/communities')}
              className="flex-1 py-3 border border-slate-200 text-slate-700 rounded-xl font-medium hover:bg-slate-50">取消</button>
